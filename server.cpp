...
void processProtocol(IClients client)
{
    ...
    char action, client_buffer[STR_LENGTH], file_buffer[STR_LENGTH];
    string block;
    while (1)
    {
        int sd_friend = -1;
        ...
        else if (action == 'F')
        {
            ...
            cout << nickname_friend << endl;
            cout << filename << endl;
            cout << size_content_str << endl;
            cout << size_content << endl;
            ...
            std::ofstream outputFile(filename, ios::out | std::ios::binary);
